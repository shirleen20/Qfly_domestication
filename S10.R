...
# Neutral lipids category for variables abundance, CC and DB
...
# We use this analysis to generate Figure 2, Tables 2 and 3.
...
#_____________________________Generate marginal means using three weights for source population analysis______________________________
...
Abundance.global.1Day <- DF_glm01_1 %>% 
  ...
  bind_rows(.id = "id") %>%  dplyr::filter(id == "NLs") %>% 
...
Abundance.global.1Dayplot.NL <- Abundance.global.1Day %>% 
  ...
  theme_bw() +
  scale_y_continuous(limits = c(-2, 2), labels = scales::number_format(accuracy = 0.1)) +
  #facet_wrap(~id) +
  theme(axis.title   = element_text(face = "bold"),
        axis.text    = element_text(face = "bold"),
        axis.text.y = element_text(size = 7),
        plot.caption = element_text(hjust = 0)) +
  #ggtitle("Percent abundance Day 1")+ theme(plot.title = element_text(size = 8, face = "bold", hjust = 0.5))+
...
Abundance.global.19Dayplot.NL <- Abundance.global.19Day %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  ...
  theme(axis.title   = element_text(face = "bold"),
        ...
        axis.text.y = element_text(size = 7),
        plot.caption = element_text(hjust = 0)) +
  #ggtitle("Percent abundance Day 19")+ theme(plot.title = element_text(size = 8, face = "bold", hjust = 0.5))+
  geom_text_repel(nudge_x = c(0, 0, 0), nudge_y = c(1.3, 1.3, 1.3), color = "black", segment.color = "transparent", size = 3)+
  theme(legend.position = "none")+
  theme(axis.title.x =element_blank()) + theme(axis.title.y =element_blank()) + theme(axis.text.x =element_blank()) +
  scale_colour_manual(values = c("ct" = "red", "cbr" = "green", "syd" = "blue", "s06" = "blue")) 
...
#___________________Variable: Mean carbon chain length in NL lipid category_________________________
...
# Day 1 NL CC global mean reduced model
...
# we will use only global mean and not all three weights to extract emmeans since Wt:Time inyteraction was not sig

CC.global.1Day.NL <- DF_glm02_1 %>% 
  split(.$Type) %>%  
  map(~ lm(Mean.Carbon.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        ...
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(str_detect(id , "NLs")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

CC.global.1Dayplot.NL <- CC.global.1Day.NL %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  theme_bw() +
  scale_y_continuous(limits = c(-1.1, 1.1), labels = scales::number_format(accuracy = 0.1)) +
  #facet_wrap(~id) +
  theme(axis.title   = element_text(face = "bold"),
        axis.text    = element_text(face = "bold"),
        axis.text.y = element_text(size = 7),
  ...
  geom_text_repel(nudge_x = c(0, 0, 0), nudge_y = c(0.52, 0.52, 0.52), color = "black", segment.color = "transparent", size = 3)+
  theme(legend.position = "none")+
  theme(axis.title.x =element_blank()) + theme(axis.title.y =element_blank()) + theme(axis.text.x =element_blank()) +
  scale_colour_manual(values = c("ct" = "red", "cbr" = "green", "syd" = "blue", "s06" = "blue")) 
...
# Day 1 NL DB global mean reduced model
...
DB.global.1Dayplot.NL <- DB.global.1Day %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  theme_bw() +
  scale_y_continuous(limits = c(-1.0, 2.0), labels = scales::number_format(accuracy = 0.1)) +
  #facet_wrap(~id) +
  theme(axis.title   = element_text(face = "bold"),
        axis.text    = element_text(face = "bold"),
        axis.text.y = element_text(size = 7),
...
#ggsave(plot = DB.global.1Dayplot.NL, width = 50, height = 30, units = "mm", dpi = 300,filename = "ScriptManuscript2/Figures/Figure2/DB_NLglobalD1.jpg")
