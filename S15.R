...
# We use this analysis to generate Figure 4 and 5, Tables 2 and 3.
...
#___________________________Variable: Abundance each class_________________________
...
# find emmeans using global fly Weight, bottom 5% wt (thin fly wt) and top 5% weight (fat fly wt) for Day 1 data
# use a reduced model to Generate marginal means 

Abundance.global.1Day.DGe <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  ...
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("DG e")) %>% 
...
Abundance.global.1Dayplot.DGe <- Abundance.global.1Day.DGe %>% 
...
Abundance.global.1Day.TGe <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("TG e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

Abundance.global.1Dayplot.TGe <- Abundance.global.1Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
Abundance.global.1Day.PCe <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("PC e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

Abundance.global.1Dayplot.PCe <- Abundance.global.1Day.PCe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
Abundance.global.1Day.PEe <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("PE e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

Abundance.global.1Dayplot.PEe <- Abundance.global.1Day.PEe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
Abundance.global.1Day.PEp <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("PE p")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

Abundance.global.1Dayplot.PEp <- Abundance.global.1Day.PEp %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  ...
  theme(axis.title   = element_text(face = "bold"),
        ...
        axis.text.y = element_text(size = 7),
        plot.title = element_text(hjust = 0.5)) +
  #ylab("Estimated marginal mean Category\ global mean fly weight") +
...
Abundance.global.1Day.PSe <- DF_glm01_1 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("PS e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

Abundance.global.1Dayplot.PSe <- Abundance.global.1Day.PSe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
Abundance.global.19Day.TGe <- DF_glm01_19 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
...
Abundance.global.19Dayplot.TGe <- Abundance.global.19Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
Abundance.global.19Day.PEp <- DF_glm01_19 %>% 
  split(.$SubClass) %>% 
  map(~ lm(Percentage.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 4.59)) %>% 
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
...
Abundance.global.19Dayplot.PEp <- Abundance.global.19Day.PEp %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
#___________________Variable: Mean carbon chain length in lipid class_________________________
...
#______Day 19 AlkylCC TGe______

# use full model for TGe Day 19 Alkyl

AlkylCC.global.19Day.TGe <- ClassCCAlkyl_19 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.CarbonAlkyl.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
        ...
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(str_detect(id , "TG e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

AlkylCC.global.19Dayplot.TGe <- AlkylCC.global.19Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
AlkylCC.bottom.19Day.TGe <- ClassCCAlkyl_19 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.CarbonAlkyl.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
...
AlkylCC.bottom.19Dayplot.TGe <- AlkylCC.bottom.19Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
AlkylCC.top.19Day.TGe <- ClassCCAlkyl_19 %>% 
  split(.$SubClass) %>%    
  map(~ lm(Mean.CarbonAlkyl.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
...
AlkylCC.top.19Dayplot.TGe <- AlkylCC.top.19Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
#___
AcylCC.global.1Day.TGe <- ClassCCAcyl_1 %>% 
  split(.$SubClass) %>%  
...
AcylCC.global.1Dayplot.TGe <- AcylCC.global.1Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
#___Day 1 AlkylDB TGe______

AlkylDB.global.1Day.TGe <- ClassDBAlkyl_1 %>% 
  split(.$SubClass) %>%  
...
AlkylDB.global.1Dayplot.TGe <- AlkylDB.global.1Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
AlkylDB.bottom.1Day.TGe <- ClassDBAlkyl_1 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.BondAlkyl.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
...
AlkylDB.bottom.1Dayplot.TGe <- AlkylDB.bottom.1Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
AlkylDB.top.1Day.TGe <- ClassDBAlkyl_1 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.BondAlkyl.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
...
AlkylDB.top.1Dayplot.TGe <- AlkylDB.top.1Day.TGe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
#___Day 1 AlkylDB PEe_____

AlkylDB.global.1Day.PEe <- ClassDBAlkyl_1 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.BondAlkyl.t ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        emmeans(~LineTime, at = list(Weight = 1.96)) %>%
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>%  dplyr::filter(str_detect(id , "PE e")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

AlkylDB.global.1Dayplot.PEe <- AlkylDB.global.1Day.PEe %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
#ggsave(plot = AlkylDB.global.1Dayplot.PEe, width = 50, height = 30, units = "mm", dpi = 300,filename = "ScriptManuscript2/Figures/Figure5/DB_AlkylchainPEeglobalD1.jpg")
