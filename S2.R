...
# write.csv("ScriptManuscript2/MainTables/Lipid abundance in each sample.csv")
...
#___________________________Calculate total lipid abundance each strain__________________________________________
...
DF03 <- DF.main %>% 
  ...
  dplyr::select(Line, Time, Age, Total.Conc, se)


# format and save as csv

DF03A <- DF03 %>% 
  dplyr::mutate(Total.Conc = format(round(.$Total.Conc, 2), nsmall = 2)) %>% 
  dplyr::mutate(se = format(round(.$se, 2), nsmall = 2)) %>% 
  tidyr::unite("AbSE", Total.Conc:se, sep = "±") %>% 
  tidyr::unite("Strain", Line:Time, sep = "") %>%
  pivot_wider(names_from = c(Strain, Age), values_from = AbSE) %>% 
  dplyr::select("ctNew_1 Day","ctNew_19 Day", "ctOld_1 Day", "ctOld_19 Day", "cbrNew_1 Day", "cbrNew_19 Day", 
                "cbrOld_1 Day", "cbrOld_19 Day", "sydNew_1 Day", "sydNew_19 Day", "sydOld_1 Day", "sydOld_19 Day", 
                "s06Older_1 Day", "s06Older_19 Day") #%>% 
 # write.csv("ScriptManuscript2/MainTables/Total abundance in the seven strains.csv")


#_____________________Calculate lipid abundance in each category for each strain_____________________________
...
Figure1 <- grid.arrange(plotA, plotB, legend, ncol=2, nrow = 2, layout_matrix = rbind(c(1,2), c(3,3)),
...
Figure2 <- grid.arrange(plotC, plotD, legend, ncol=2, nrow = 2, layout_matrix = rbind(c(1,2), c(3,3)),
                        widths = c(2.7, 2.7), heights = c(2.5, 0.2))
