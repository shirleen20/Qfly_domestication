...
DBAlkyl <- Composition %>% 
  ...
  summarySE(groupvars = c("LineTime", "SubClass", "Age"), measurevar = "Mean.BondAlkyl") %>% 
  dplyr::select(LineTime, SubClass, Age, Mean.BondAlkyl)
...
DF_glm01_D1 <- DBAlkyl %>%
  ...
  pivot_wider(names_from = c(LineTime), values_from = Mean.BondAlkyl) %>% 
  ...
  dplyr::select(-2:-8) %>% 
  pivot_longer(cols = CN.SD_new:CT.SD_old, names_to = "Comparisons", values_to="Mean.BondAlkyl") %>% 
  ...
  dplyr::select(SubClass, Line, Mean.BondAlkyl, Time) %>% 
  pivot_wider(names_from = c(Time), values_from = Mean.BondAlkyl) 


M1_1DBAlkyl <- DF_glm01_D1 %>% split(.$SubClass) %>% 
  ...
  dplyr::mutate(Test = "DBAlkyl_H1D1")
#write_excel_csv("ScriptManuscript2/T.test/Mean.DBAlkylD1_H1.csv")

# Do FDR corrections for M1_1

FDRcorrected.M1_1DBAlkyl <- M1_1DBAlkyl %>% 
...
# T-test
M1_19DBAlkyl <- DF_glm01_D19 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAlkyl_H1D19")
#write_excel_csv("ScriptManuscript2/T.test/Mean.DBAlkylD19_H1.csv")

FDRcorrected.M1_19DBAlkyl <- M1_19DBAlkyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
DF_glm02_D1 <- DBAlkyl %>%
  ...
  dplyr::select(-2:-9) %>% 
  pivot_longer(cols = CN.S06_new:SD.S06_Old, names_to = "Comparisons", values_to="Mean.BondAlkyl") %>% 
...
# T-test
M2_1DBAlkyl <- DF_glm02_D1 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAlkyl_H2D1")
#write_excel_csv("ScriptManuscript2/T.test/Mean.BondAlkylD1_H2.csv")

FDRcorrected.M2_1DBAlkyl <- M2_1DBAlkyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))


DF_glm02_D19 <- DBAlkyl %>%
  dplyr::filter(Age == "19 Day") %>% 
  #dplyr::filter(LineTime!= "s06_Older") %>% 
  pivot_wider(names_from = c(LineTime), values_from = Mean.BondAlkyl) %>% 
  dplyr::mutate(CN.S06_new = abs(cbr_New-s06_Older), CT.S06_new = abs(ct_New-s06_Older), SD.S06_new = abs(syd_New-s06_Older)) %>% 
...
# T-test
M2_19DBAlkyl <- DF_glm02_D19 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAlkyl_H2D19")
#write_excel_csv("ScriptManuscript2/T.test/Mean.BondAlkylD19_H2.csv")

FDRcorrected.M2_19DBAlkyl <- M2_19DBAlkyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
DBAcyl <- Composition %>% 
  ...
  tidyr::unite(LineTime, Line, Time, remove = FALSE) %>% 
  summarySE(groupvars = c("LineTime", "SubClass", "Age"), measurevar = "Mean.BondAcyl") %>% 
  dplyr::select(LineTime, SubClass, Age, Mean.BondAcyl)
...
DF_glm03_D1 <- DBAcyl %>%
  dplyr::filter(Age == "1 Day") %>% 
  dplyr::filter(LineTime!= "s06_Older") %>% 
  pivot_wider(names_from = c(LineTime), values_from = Mean.BondAcyl) %>% 
  dplyr::mutate(CN.SD_new = abs(cbr_New-syd_New), CN.CT_new = abs(cbr_New-ct_New), CT.SD_new = abs(ct_New-syd_New)) %>% 
  dplyr::mutate(CN.SD_old = abs(cbr_Old-syd_Old), CN.CT_old = abs(cbr_Old-ct_Old), CT.SD_old = abs(ct_Old-syd_Old)) %>%
  dplyr::select(-2:-8) %>% 
  pivot_longer(cols = CN.SD_new:CT.SD_old, names_to = "Comparisons", values_to="Mean.BondAcyl") %>% 
  dplyr::mutate(Time = case_when(Comparisons %in% c('CN.SD_new', 'CN.CT_new', 'CT.SD_new') ~ "New",
                                 Comparisons %in% c("CN.SD_old","CN.CT_old","CT.SD_old") ~ "Old")) %>%
  tidyr::separate(col = Comparisons, into = c("Line", NA), "_", remove = TRUE) %>% 
  dplyr::select(SubClass, Line, Mean.BondAcyl, Time) %>% 
  pivot_wider(names_from = c(Time), values_from = Mean.BondAcyl) 


M1_1DBAcyl <- DF_glm03_D1 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAcyl_H1D1")
#write_excel_csv("ScriptManuscript2/T.test/Mean.DBAcylD1_H1.csv")

FDRcorrected.M1_1DBAcyl <- M1_1DBAcyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
# T-test
M1_19DBAcyl <- DF_glm03_D19 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAcyl_H1D19")
#write_excel_csv("ScriptManuscript2/T.test/Mean.DBAcylD19_H1.csv")

FDRcorrected.M1_19DBAcyl <- M1_19DBAcyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
DF_glm04_D1 <- DBAcyl %>%
  ...
  dplyr::mutate(CN.S06_old = abs(cbr_Old-s06_Older), CT.S06_Old = abs(ct_Old-s06_Older), SD.S06_Old = abs(syd_Old-s06_Older)) %>% 
  dplyr::select(-2:-9) %>% 
  pivot_longer(cols = CN.S06_new:SD.S06_Old, names_to = "Comparisons", values_to="Mean.BondAcyl") %>% 
  dplyr::mutate(Time = case_when(Comparisons %in% c('CN.S06_new', 'CT.S06_new', 'SD.S06_new') ~ "New",
                                 Comparisons %in% c("CN.S06_old","CT.S06_Old","SD.S06_Old") ~ "Old"))%>%
  tidyr::separate(col = Comparisons, into = c("Line", NA), "_", remove = TRUE) %>% 
  dplyr::select(SubClass, Line, Mean.BondAcyl, Time) %>% 
...
# T-test
M2_1DBAcyl <- DF_glm04_D1 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAcyl_H2D1")
#write_excel_csv("ScriptManuscript2/T.test/Mean.BondAcylD1_H2.csv")

FDRcorrected.M2_1DBAcyl <- M2_1DBAcyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
# T-test
M2_19DBAcyl <- DF_glm04_D19 %>% split(.$SubClass) %>% 
  map(~ t.test(.$New, .$Old, paired=TRUE, conf.level=0.95, data = .x) %>% 
        broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  dplyr::select(1, 2, 4, 6, 7, 10) %>% 
  dplyr::mutate(Test = "DBAcyl_H2D19")
#write_excel_csv("ScriptManuscript2/T.test/Mean.BondAcylD19_H2.csv")

FDRcorrected.M2_19DBAcyl <- M2_19DBAcyl %>% 
  dplyr::select(id, 'p.value') %>% 
  dplyr::mutate(p.value = p.adjust((p.value), method = "fdr", n = length(p.value))) %>% 
  dplyr::mutate(p.value = stars.pval(p.value))
...
# Save results
T.test.CC <- rbind(M1_1DBAlkyl, M1_19DBAlkyl, M2_1DBAlkyl, M2_19DBAlkyl,
                   M1_1DBAcyl, M1_19DBAcyl, M2_1DBAcyl, M2_19DBAcyl) #%>% 
  #write_excel_csv("ScriptManuscript2/T.test/DB_ether.csv")
