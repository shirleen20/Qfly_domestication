...
# Neutral lipids class for variables abundance, CC and DB
...
Abundance.global.1Day <- DF_glm01_1 %>% 
  ...
  bind_rows(.id = "id") %>%  dplyr::filter(id %in% c("TG")) %>% 
...
Abundance.global.1Dayplot.TG <- Abundance.global.1Day %>% 
...
#_______TG Day 1 CC global mean_____
...
CC.global.1Dayplot.TG <- CC.global.1Day.TG %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  theme_bw() +
  scale_y_continuous(limits = c(-1.3, 1.3), labels = scales::number_format(accuracy = 0.1)) +
  #facet_wrap(~id) +
  theme(axis.title   = element_text(face = "bold"),
        axis.text    = element_text(face = "bold"),
        axis.text.y = element_text(size = 7),
...
CC.global.1Day.DG <- DF_glm02_1 %>% 
  split(.$SubClass) %>%  
  map(~ lm(Mean.Carbon.t ~ as.numeric(Weight) * LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>%
        emmeans(~interaction(LineTime), at = list(Weight = 1.96)) %>%       
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") %>% dplyr::filter(id %in% c("DG")) %>% 
  tidyr::separate(col = LineTime, into = c("Line", "Time"), "_", remove = FALSE)

CC.global.1Dayplot.DG  <- CC.global.1Day.DG  %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
CC.bottom.1Dayplot.DG  <- CC.bottom.1Day.DG  %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  ...
  geom_text_repel(nudge_x = c(0, 0, 0), nudge_y = c(1.6, 1.6, 1.6), color = "black", segment.color = "transparent", size = 3)+
  theme(legend.position = "none")+ theme(axis.title.x =element_blank())+
  theme(axis.title.x =element_blank()) + theme(axis.title.y =element_blank()) + theme(axis.text.x =element_blank()) +
  scale_colour_manual(values = c("ct" = "red", "cbr" = "green", "syd" = "blue", "s06" = "blue")) 
...
CC.top.1Dayplot.DG  <- CC.top.1Day.DG  %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  = lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
...
DB.global.1Dayplot.TG <- DB.global.1Day.TG %>% 
  dplyr::mutate(LineTime = factor(as.factor(LineTime), levels = c("ct_New","ct_Old","syd_New","syd_Old","s06_Older","cbr_New","cbr_Old"))) %>%
  ggplot(aes(x = LineTime, y = emmean, label = .group)) +
  geom_point(mapping = aes(color = Line, shape  = Time), size = 2, position = pd) +
  geom_errorbar(aes(ymin  =  lower.CL, ymax =  upper.CL, color = Line), width =  0.1, size  =  0.4, position = pd) +
  theme_bw() +
  scale_y_continuous(limits = c(-1.2, 2), labels = scales::number_format(accuracy = 0.1)) +
  #facet_wrap(~id) +
  theme(axis.title   = element_text(face = "bold"),
        axis.text    = element_text(face = "bold"),
        axis.text.y = element_text(size = 7),
...
#ggsave(plot = DB.global.1Dayplot.TG, width = 50, height = 30, units = "mm", dpi = 300,filename = "ScriptManuscript2/Figures/Figure2/DB_TGglobalD1.jpg")
