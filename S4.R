#_____________________________________24/01/2025______________________________________

# Here we perform univariate analysis (Anova lm) on variables Carbon content and Double bond content for ester
# lipid category: Neutral lipids, phospholipids
# we 1st fit a full model and check if the interaction term is sig after FDR corrections, if not than 
# we fit a reduced model and we also generate marginal means for variables where strain was significant
...
# Calculate the mean CC present in each lipid category in each of the four lipid categories
...
DF_glm01 <- Composition %>% 
  ...
  dplyr::ungroup() %>%
  dplyr::select(Samples, Line, Time, Age, Type, Weight, Mean.Carbon) %>% 
  unique() %>% 
  tidyr::unite(Type.Age, Type, Age, remove = FALSE) %>% 
  dplyr::select(Type.Age, Samples, Line, Type, Age, Time, Mean.Carbon, Weight) %>% 
...
FDRcorrected.Carbon.fullmodel <- DF.p.values %>% 
...
FDRcorrected.Carbon.reducedmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`(Intercept)` = stars.pval(`(Intercept)`)) %>% 
...
### NOTE: WE WILL NEED TO USE REDUCE MODELS SINCE NONE OF THE INTERCATION TERMS ARE SIGNIFICANT

# save Anova results after FDR corrections reduced model
rbind(FDRcorrected.Carbon.fullmodel) #%>% write_excel_csv("ScriptManuscript2/Carbon/FDRcorrected.fullmodelCategoriesCC.csv")
rbind(FDRcorrected.Carbon.reducedmodel) #%>% write_excel_csv("ScriptManuscript2/Carbon/FDRcorrected.reducedmodelCategories.CC.csv")

# save Anova results for CC full model 
M1 <- by_Type.Age %>% 
  map(~ lm(Mean.Carbon ~ as.numeric(Weight)* LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        car::Anova(type = 3) %>% broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
...
#write_excel_csv("ScriptManuscript2/Carbon/Anovaresults.fullmodelCategories.CC.csv")

# save Anova results for CC Reduced model 
M1.R <- by_Type.Age %>% 
  map(~ lm(Mean.Carbon ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        car::Anova(type = 3) %>% broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  ...
  unite("Str", "F_LineTime", "Str." , sep = "(") %>% 
  dplyr::mutate(Str = paste0(Str,")")) #%>%
  #write_excel_csv("ScriptManuscript2/Carbon/Anovaresults.reducedmodelCategories.CC.csv")


#_________________________Generate marginal means Carbon content__________________________________
...
M1.R <- by_Type.Age %>% 
  ...
  bind_rows(.id = "id") #%>% 
  #write_excel_csv("ScriptManuscript2/Carbon/EmmeansCategoryReducedmodel.CC.csv")

#_______________________________Generate Master Table with Reduced Model for CC______________________________

rm(list=ls())

#_____________________
Emmeans <- readxl::read_excel("ScriptManuscript2/Carbon/EmmeansCategoryReducedmodel.CC.xlsx")
...
A <- DF.p.Emmeans %>% 
  ...
  #write_excel_csv("ScriptManuscript2/Carbon/Mastertable_Reducedmodel.Category.CC.csv")

#______________________________________________________________________________________________________________________________________


#___________________________Variable: Mean double bond in lipid categories ________________________
...
DF_glm02 <- Composition %>% 
  ...
  dplyr::ungroup() %>%
  dplyr::select(Samples, Line, Time, Age, Type, Weight, Mean.Bond) %>% 
  unique() %>% 
  tidyr::unite(LineTime, Line, Time, remove = FALSE) %>% 
  tidyr::unite(Type.Age, Type, Age, remove = FALSE) %>% 
  dplyr::select(Type.Age, Samples, Line, Type, Age, Time, Mean.Bond, Weight) %>% 
  tidyr::unite(LineTime, Line, Time, remove = FALSE) 

by_Type.Age <- DF_glm02 %>% split(.$Type.Age)
...
# Do FDR corrections for variables for full model Double bond content variable
...
FDRcorrected.Bond.fullmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`as.numeric(Weight):LineTime` = p.adjust(DF.p.values$`as.numeric(Weight):LineTime`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight):LineTime`))) %>% 
...
FDRcorrected.Bond.reducedmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`(Intercept)` = stars.pval(`(Intercept)`)) %>% 
...
# save Anova results after FDR corrections reduced model
rbind(FDRcorrected.Bond.fullmodel) #%>% write_excel_csv("ScriptManuscript2/DoubleBond/FDRcorrected.fullmodelCategoriesDB.csv")
rbind(FDRcorrected.Bond.reducedmodel) #%>% write_excel_csv("ScriptManuscript2/DoubleBond/FDRcorrected.reducedmodelCategories.DB.csv")

# NOTE: NONE OF THE INTERACTION TERMS ARE SIG SO WE WILL USE A REDUCED MODEL for DB 

# save Anova results for DB full model 
M2 <- by_Type.Age %>% 
  map(~ lm(Mean.Bond ~ as.numeric(Weight)* LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        car::Anova(type = 3) %>% broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>% 
  ...
  unite("Wt.Str", "F_as.numeric(Weight):LineTime", "WtStr.", sep = "(") %>% 
  dplyr::mutate("Wt.Str" = paste0(Wt.Str,")")) #%>%
#write_excel_csv("ScriptManuscript2/DoubleBond/Anovaresults.fullmodelCategories.DB.csv")

# save Anova results for DB Reduced model 
...
M2.R <- by_Type.Age %>% 
  ...
  unite("Str", "F_LineTime", "Str." , sep = "(") %>% 
  dplyr::mutate(Str = paste0(Str,")")) #%>%
  #write_excel_csv("ScriptManuscript2/DoubleBond/Anovaresults.reducedmodelCategories.DB.csv")

#_________________________Generate marginal means Double Bond content _____________________________
...
M2.R <- by_Type.Age %>% 
  map(~ lm(Mean.Bond ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        ...
        cld(Letter = "abcdefgh") %>% as_tibble()) %>% 
  bind_rows(.id = "id") #%>% 
  #write_excel_csv("ScriptManuscript2/DoubleBond/EmmeansCategoryReducedmodel.DB.csv")


#___________________________Generate Master Table with Reduced Model for DB_______________________

rm(list=ls())

#_____________________

Emmeans <- readxl::read_excel("ScriptManuscript2/DoubleBond/EmmeansCategoryReducedmodel.DB.xlsx")
...
A <- DF.p.Emmeans %>% 
  ...
  dplyr::filter(Unit !=  " NA") %>% 
  tidyr::pivot_wider(names_from = LineTime, values_from = P) #%>% 
  #write_excel_csv("ScriptManuscript2/DoubleBond/MastertableReducedmodel.Category.DB.csv")
