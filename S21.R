...
# Here we perform t-tests on variables double bond content in ester lipids
...
#create dataframe for mean carbon and double bond calculation in lipid category_____________________
...
DF_glm01 <-  Composition %>% 
  ...
  tidyr::unite(LineTime, Line, Time, remove = FALSE)%>% 
  summarySE(groupvars = c("LineTime", "SubClass", "Age"), measurevar = "Mean.Bond") %>% 
  dplyr::select(LineTime, SubClass, Age, Mean.Bond) 
...
DF_glm01_D1 <- DF_glm01 %>%
  ...
  pivot_wider(names_from = c(LineTime), values_from = Mean.Bond) %>% 
  ...
  dplyr::select(-2:-8) %>% 
  pivot_longer(cols = CN.SD_new:CT.SD_old, names_to = "Comparisons", values_to="Mean.Bond") %>% 
  ...
  dplyr::select(SubClass, Line, Mean.Bond, Time) %>% 
  pivot_wider(names_from = c(Time), values_from = Mean.Bond) 
...
M1_1 <- DF_glm01_D1 %>% split(.$SubClass) %>% 
  ...
  dplyr::mutate(Test = "DB_H1D1")
#write_excel_csv("ScriptManuscript2/T.test/Mean.BondD1_H1.csv")
...
DF_glm02_D1 <- DF_glm01 %>%
  ...
  dplyr::select(-2:-9) %>% 
  pivot_longer(cols = CN.S06_new:SD.S06_Old, names_to = "Comparisons", values_to="Mean.Bond") %>% 
...
T.test.CC <- rbind(M1_1, M1_19, M2_1, M2_19) #%>% 
  #write_excel_csv("ScriptManuscript2/T.test/DB_ester.csv")
