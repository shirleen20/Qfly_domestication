#___________________________________________24/01/2025___________________________________________

# Here we perform univariate analysis (Anova lm) for variable PE/PC ratio and percentage abundance each class  
# we 1st fit a full model and check if the interaction term is sig after FDR corrections, if not than 
# we fit a reduced model and generate marginal means for variables where strain was significant
# We use this analysis to generate Tables 1, S7 and S10.
...
by_Age <- DF_glm00 %>% split(.$Age)

# fit a full model to test significant terms
M1 <- by_Age %>% 
  map(~ lm(Ratio1 ~ as.numeric(Weight)* LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
...
FDRcorrected.PEPC.fullmodel <- DF.p.values %>% 
...
# NOTE NO SIG WT:STR INTERACTION SO WE WILL USE REDUCED MODEL FOR VARIABLE PE/PC ratio

# save Anova results after FDR corrections full model
rbind(FDRcorrected.PEPC.fullmodel) #%>% write_excel_csv("ScriptManuscript2/Abundance/FDRcorrected.fullmodel.PEPC.csv")
...
M1 <- by_Age %>% 
  ...
  #write_excel_csv("ScriptManuscript2/Abundance/Anovaresults.fullmodel.PEPC.csv")

# SAVE EMMEANS RESULTS for reduced models
rbind(M1.Emmeans) #%>% write_excel_csv("ScriptManuscript2/Abundance/Emmeans_PEPC.csv")
...
FDRcorrected.PEPC.reducedmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`(Intercept)` = stars.pval(`(Intercept)`)) %>% 
...
rbind(FDRcorrected.PEPC.reducedmodel) #%>% write_excel_csv("ScriptManuscript2/Abundance/FDRcorrected.reducedmodel.PEPC.csv")
...
M1.R <- by_Age %>% 
  ...
  unite("Str", "F_LineTime", "Str." , sep = "(") %>% 
  dplyr::mutate(Str = paste0(Str,")")) #%>%
  #write_excel_csv("ScriptManuscript2/Abundance/Anovaresults.reducedmodel.PEPC.csv")
...
# Generate marginal means using reduced model
...
rbind(M1.R.Emmeans) #%>% write_excel_csv("ScriptManuscript2/Abundance/Emmeans.Class.csv")
...
# 1. Percent Conc variable
DF.p.values <- M1 %>% 
  dplyr::select(id, term, 'p.value') %>% 
  filter(!str_detect(term, "Residuals")) %>% 
  pivot_wider(names_from = term, values_from = p.value) 

FDRcorrected.Conc.fullmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`as.numeric(Weight):LineTime` = p.adjust(DF.p.values$`as.numeric(Weight):LineTime`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight):LineTime`))) %>% 
...
#NOTE: SINCE NONE OF THE WT:STR INTERACTION IS SIG, WE WILL USE A REDUCED MODEL

# save Anova results after FDR corrections full model
rbind(FDRcorrected.Conc.fullmodel) #%>% write_excel_csv("ScriptManuscript2/Abundance/FDRcorrected.fullmodel.ClassAb.csv")
...
M1 <- by_SubClass.Age %>% 
  ...
  unite("Wt.Str", "F_as.numeric(Weight):LineTime", "WtStr.", sep = "(") %>% 
  dplyr::mutate("Wt.Str" = paste0(Wt.Str,")")) #%>% 
  #write_excel_csv("ScriptManuscript2/Abundance/Anovaresults.fullmodel.Class.csv")
...
FDRcorrected.Conc.reducedmodel <- DF.p.values %>% 
  dplyr::mutate(`(Intercept)` = p.adjust(DF.p.values$`(Intercept)`, method = "fdr", n = length(DF.p.values$`(Intercept)`))) %>% 
  dplyr::mutate(`as.numeric(Weight)` = p.adjust(DF.p.values$`as.numeric(Weight)`, method = "fdr", n = length(DF.p.values$`as.numeric(Weight)`))) %>% 
  dplyr::mutate(LineTime = p.adjust(DF.p.values$LineTime, method = "fdr", n = length(DF.p.values$LineTime))) %>% 
  dplyr::mutate(`(Intercept)` = stars.pval(`(Intercept)`)) %>% 
...
# save Anova results after FDR corrections reduced model
rbind(FDRcorrected.Conc.reducedmodel) #%>% write_excel_csv("ScriptManuscript2/Abundance/FDRcorrected.reducedmodel.ClassAb.csv")

# save Anova results for full model for dataset 1
M1.R <- by_SubClass.Age %>% 
  map(~ lm(Percentage ~ as.numeric(Weight) + LineTime, data = .x, contrasts = list(LineTime = contr.sum)) %>% 
        car::Anova(type = 3) %>% broom::tidy()) %>% bind_rows(.id = "id") %>% 
  dplyr::mutate(Stars = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*",ifelse(p.value < 0.1, ".",""))))) %>%  
  ...
  unite("Str", "F_LineTime", "Str." , sep = "(") %>% 
  dplyr::mutate(Str = paste0(Str,")")) #%>%
  #write_excel_csv("ScriptManuscript2/Abundance/Anovaresults.reducedmodel.Class.csv")
